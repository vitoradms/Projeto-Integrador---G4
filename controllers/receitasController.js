const { Receita, Comentarios } = require("../database/models");
...
const receitasController = {
  ...
  salvar: async (req, res) => {
    ...
    if (listaDeErros.isEmpty()) {
      ...
      const foto_receita = req.file
        ? req.file.filename
        : res.send("Por favor, adicione uma foto a sua receita!");
      const ingredientes = [];
      ...
      res.render("salvarReceita", {
        receita: null,
        errors: listaDeErros.errors,
        old: req.body,
      });
    }
  },
  ...
  buscarReceita: async (req, res) => {
    ...
    const receitas = await Receita.findAll({
        include: [
            { association: 'ingredientes', where: { nome: ingrediente}}
        ]
    })
